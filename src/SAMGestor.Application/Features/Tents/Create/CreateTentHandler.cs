...
) : IRequestHandler<CreateTentCommand, CreateTentResponse>
{
    public async Task<CreateTentResponse> Handle(CreateTentCommand cmd, CancellationToken ct)
    {
        ...
        // 2) Checa lock global (usar BusinessRuleException para 400)
        ...
        if (lockedProp is not null && lockedProp.PropertyType == typeof(bool))
        {
            ...
            if (isLocked)
                throw new BusinessRuleException("As barracas estão travadas para este retiro.");
        }

        // 3) Converter o Number (string -> int) — se o pipeline de validação estiver ativo,
        //    isto já foi garantido, mas mantemos por segurança.
        ...
        // 4) Unicidade: (RetreatId, Category, Number)
        var exists = await tentRepo.ExistsNumberAsync(cmd.RetreatId, cmd.Category, number, ignoreId: null, ct);
        if (exists)
        ...
        // 7) Bump na versão de barracas (se existir)
