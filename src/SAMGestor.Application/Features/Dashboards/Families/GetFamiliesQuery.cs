...
public sealed record GetFamiliesQuery(Guid RetreatId, int Top = 10) : IRequest<FamiliesListDto>;
...
public sealed class GetFamiliesHandler : IRequestHandler<GetFamiliesQuery, FamiliesListDto>
{
    private readonly IReportingReadDb _db;
    public GetFamiliesHandler(IReportingReadDb db) => _db = db;

    public async Task<FamiliesListDto> Handle(GetFamiliesQuery request, CancellationToken ct)
    {
        ...
        return new FamiliesListDto
        {
            Total = grouped.Count,
            Items = grouped.Take(request.Top).ToArray()
        };
    }
