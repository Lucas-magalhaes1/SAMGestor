...
public sealed class RegisterServiceManualPaymentCommandValidator : AbstractValidator<RegisterServiceManualPaymentCommand>
{
    public RegisterServiceManualPaymentCommandValidator()
    {
        RuleFor(x => x.ServiceRegistrationId)
            .NotEmpty().WithMessage("ID da inscrição de serviço é obrigatório");
