...
```
SAMGestor/
├── src/SAMGestor.API              # API principal (Core)
├── src/SAMGestor.Application      # Lógica de aplicação (CQRS/MediatR)
├── src/SAMGestor.Domain           # Entidades e regras de negócio
├── src/SAMGestor.Infrastructure   # Persistência e infraestrutura
├── services/payment/              # Microserviço de Pagamentos
├── services/notification/         # Microserviço de Notificações
└── shared/contracts/              # Contratos de eventos compartilhados
```
...
| Regra | Descrição |
|-------|-----------|
| **RN001** | CPF bloqueado não pode se inscrever |
| **RN002** | CPF só pode ter uma inscrição ativa por retiro |
| **RN003** | Termos devem ser aceitos obrigatoriamente |
| **RN004** | Gravidez só pode ser informada para gênero feminino |
| **RN005** | Idade mínima e máxima podem ser configuradas por retiro |
...
## 🎲 Módulo de Contemplação (Sorteio)

### Visão Geral

O módulo de contemplação realiza a seleção aleatória de participantes respeitando:
- Vagas disponíveis por gênero
- Quotas regionais (Oeste vs Outras Regiões)
- Seleções manuais prioritárias

### Fluxo de Contemplação

```mermaid
graph TD
    A[Iniciar Sorteio] --> B[Verificar Retiro]
    B --> C{Contemplação Fechada?}
    C -->|Sim| D[Erro: Contemplação Encerrada]
    C -->|Não| E[Calcular Vagas Disponíveis]
    E --> F[Buscar Candidatos NotSelected]
    F --> G[Separar por Gênero]
    G --> H[Embaralhar Listas]
    H --> I[Selecionar até Limite]
    I --> J{Preview ou Commit?}
    J -->|Preview| K[Retornar Resultado]
    J -->|Commit| L[Atualizar Status para Selected]
    L --> M[Commit Transação]
```
...
1. **Cálculo de Capacidade**
   ```csharp
   var maleOccupied = CountByStatusesAndGender(SlotPolicy.OccupyingStatuses, Male);
   var femaleOccupied = CountByStatusesAndGender(SlotPolicy.OccupyingStatuses, Female);

   var maleCapacity = Max(0, retreat.MaleSlots - maleOccupied);
   var femaleCapacity = Max(0, retreat.FemaleSlots - femaleOccupied);
   ```
...
3. **Embaralhamento Criptográfico**
   ```csharp
   // Usa RandomNumberGenerator para garantir aleatoriedade
   Shuffler.ShuffleInPlace(malePool);
   Shuffler.ShuffleInPlace(femalePool);
   ```

4. **Seleção**
   ```csharp
   var malePick = malePool.Take(Min(maleCapacity, malePool.Count));
   var femalePick = femalePool.Take(Min(femaleCapacity, femalePool.Count));
   ```
...
|--------|----------|-----------|
| POST | `/api/retreats/{id}/lottery/preview` | Simula sorteio sem persistir |
| POST | `/api/retreats/{id}/lottery/commit` | Executa e persiste sorteio |
| POST | `/api/retreats/{id}/selections/{regId}` | Seleção manual de participante |

### Seleção Manual

Permite contemplar um participante específico fora do sorteio:

**Validações:**
- Retiro não pode estar com contemplação fechada
- Participante deve pertencer ao retiro
- Deve estar com status `NotSelected`
- Deve haver vagas disponíveis para o gênero

**Transação:**
- Usa isolamento `SERIALIZABLE` para evitar race conditions
- Verifica vagas novamente dentro da transação
...
| Regra | Descrição |
|-------|-----------|
| **RN101** | Contemplação fechada impede novos sorteios |
| **RN102** | Vagas são separadas por gênero (Male/Female) |
| **RN103** | Status que ocupam vaga: Selected, PendingPayment, PaymentConfirmed, Confirmed |
| **RN104** | Seleção manual respeita limite de vagas |
| **RN105** | Embaralhamento usa gerador criptográfico para garantir aleatoriedade |

### Quotas Regionais

O sistema suporta configuração de percentuais por região:
...
**Nota:** A implementação atual não aplica as quotas regionais no algoritmo de sorteio. Isso está preparado para implementação futura.

---

## 👨‍👩‍👧‍👦 Módulo de Geração de Famílias
